---
references:
  - [レイヤードアーキテクチャ](https://github.com/rakus-public/styleguide/blob/main/go/layered-architecture.md)
---
...
- 主な責務
  ...
  - HTTP / WebSocket のエンドポイント（ルーティング・ハンドラ・DTO・シリアライズ）は、DDD では Presentation / Interface Adapters 層（いわゆる web アダプタ）に置くのが定石
...
- Infrastructure 層は DB・メッセージブローカー・外部 API など技術的な詳細実装（Repository 実装やクライアント）を置く場所であり、API ハンドラ本体は置かないのが原則
